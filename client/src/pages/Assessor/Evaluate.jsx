...
function StudentList() {
  ...
  const FetchStudent = async () => {
    try {
      const res = await fetch('http://localhost:5000/api/assessor/showEvaluationStudent',
        {
          method: 'GET'
        }
      );
  ...
  const NavigateHandle = (role) => {
    navigate(`/Assessor/${role}`);
  }
                ...
                <tbody className="divide-y divide-slate-700/50 text-sm text-slate-300">
                  {data.map((dataRow, idx) => (
                    <tr key={dataRow.student_id || idx} className="hover:bg-slate-700/30 transition-colors">
                      ...
                      <td className="py-3 px-4 font-medium text-white">{`${dataRow.student_first_name || 'N/A'} ${dataRow.student_last_name || 'N/A'}`}</td>
                      ...
                      <td className="py-3 px-4 text-right">
                        <button 
                          onClick={() => NavigateHandle('Evaluate')}
