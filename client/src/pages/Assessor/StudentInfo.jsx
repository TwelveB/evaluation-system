...
function StudentList() {
  ...
  const NavigateEvaluate = (id) => {
    navigate(`/Assessor/Evaluate/${id}`);
  }
  ...
  return (
      <div className="">
        <h1 className="text-3xl font-bold text-sky-400 mb-2">รายชื่อรอบการประเมิน</h1>
                  {loading ? (
            ...
            <div className="overflow-x-auto">
              <h2>การประเมินของนักเรียน {`${data[0].student_first_name} ${data[0].student_last_name}`}</h2>
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-slate-700 text-xs font-semibold text-slate-400 uppercase tracking-wider">
                    <th className="py-3 px-4">#</th>
                    <th className="py-3 px-4">หัวข้อรายการประเมิน</th>
                    <th className="py-3 px-4">ชื่อผู้ประเมิน</th>
                ...
                <tbody className="divide-y divide-slate-700/50 text-sm text-slate-300">
                  {data.map((dataRow, idx) => (
                    <tr key={dataRow.student_id || idx} className="hover:bg-slate-700/30 transition-colors">
                      ...
                      <td className="py-3 px-4 font-medium text-white">{dataRow.evaluation_title}</td>
                      <td className="py-3 px-4 font-medium text-white">{`${dataRow.assessor_first_name || 'N/A'} ${dataRow.assessor_last_name || 'N/A'}`}</td>
                      ...
                      <td className="py-3 px-4 text-right">
                        <button 
                          onClick={() => NavigateEvaluate(dataRow.evaluation_id)}
